...
( 	input wire clk, 
	...
	output reg end_out,
	output reg kernel_en
);

//enable ==========================================================================
reg kernel_en;	
always @(posedge clk) begin 
	if ( rst || conv_and_relu_done ) kernel_en <= 1'b0;
	else if ( op_st ) kernel_en <= 1'b1;
	else kernel_en <= kernel_en;
end
...
always @(posedge clk) begin 
	...
	else if ( kernel_en == 1 && end_out == 0 ) begin 
		cnt <= cnt + 1;
	end
	else cnt <= cnt;
end
...
always @ ( posedge clk ) begin 
	if ( rst || end_out ) dout <= 15'd0;
		...
		data14 <= data15;
		if ( cnt <= 6 ) data15 <= 0;
		else if (mode == 3'b000 && cnt >=263 ) data15 <= 0;
		else if (mode == 3'b001 && cnt >=135 ) data15 <= 0;
		else if (mode == 3'b010 && cnt >=71 ) data15 <= 0;
		else if (mode == 3'b011 && cnt >=39 ) data15 <= 0;
		else data15 <= (mode == 3'b000 ) ? {in_data, 1'b0} : {1'b0, in_data} ;	//7 bit
		// *******************
